    ...
    <view class="caption-wrap">
      <i-collapse>
        <i-collapse-item
          ...
          v-for="(p,index) in person" :key="p.uid" @click="dev(index)">
          <view slot="content">
            <div class="weui-cells weui-cells_after-title" style="background-color: #f7f7f7">
              <div class="weui-cell" v-for="(d,devindex) in devList['person'+index]" :key="d.did"
                   @click="devdetail(index,devindex)">
                ...
                <span>
                  ...
                  <img v-if="d.blocking==1" src="/static/icon/onNet.png" class="position"/>
                </span>
                </div>
...
<script>
  export default {
    ...
    methods: {
      getPerson () {
        this.$api.get('/person', {'hasplan': 1}, null, r => {
          ...
          for (let i = 0, len = list.length; i < len; i++) {
            obj = list[i]
            obj['imageurl'] = _this.$api.ImgName(obj.uicon)
            _this.getDev(obj)
          }
          _this.person = list
      ...
      getDev (obj) {
        this.$api.get('/dev-plan', {'pid': obj.uid}, null, r => {
          let devList = r.data
          let dev = {}
          let planDev = []
      ...
      dev (index) {
        ...
        devList.forEach(function (dev, i) {
          ...
          plans.forEach(function (v, k) {
            ...
            v['starttimeStr'] = _this.$api.formatDate('hh:mm', new Date(v.starttime * 1000 - 8 * 3600 * 1000))
            v['endtimeStr'] = _this.$api.formatDate('hh:mm', new Date(v.endtime * 1000 - 8 * 3600 * 1000))
          })
        })
